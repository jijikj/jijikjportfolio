...
import { Cpu, ShieldCheck, Layers, KanbanSquare, Calendar, ArrowUpRight } from "lucide-react";

// Icône GitHub en SVG pour éviter les problèmes de cache avec lucide-react
      ...
      <div className="flex overflow-hidden w-full relative mb-14 before:absolute before:left-0 before:top-0 before:z-10 before:h-full before:w-20 before:bg-gradient-to-r before:from-slate-950 before:to-transparent after:absolute after:right-0 after:top-0 after:z-10 after:h-full after:w-20 after:bg-gradient-to-l after:from-slate-950 after:to-transparent">
        ...
        >
          {dupeProjets.map((projet, index) => (
            ...
            >
              <a href={projet.lienDemo} target="_blank" rel="noopener noreferrer" className="block space-y-4">
                <div className="aspect-[16/10] w-full bg-slate-950 rounded-2xl overflow-hidden mb-4 border border-slate-800">
                  <img src={projet.image} alt={projet.nom} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                </div>
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
                <div className="flex flex-wrap gap-1.5 pt-4 border-t border-slate-800/50 mt-4">
                ...
                {projet.lienGit && (
                  <div className="flex-shrink-0">
                    <a href={projet.lienGit} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white transition-colors bg-slate-800/50 hover:bg-slate-800 px-3 py-2 rounded-lg">
                      <GithubIcon size={14} />
                      Code Source
                    </a>
                  </div>
